...
Для класса \In{Enum} определён специальный синтаксис
...
а любые значения из класса \In{Enum}. Например определим
тип:
...
В этой записи мы фильтруем список \In{list} предикатом \In{p}
и преобразуем результат функцией \In{x}. Например возведём
...
пользоваться методами класса \In{Monad}. Очень
...
специальное окружение \KeyWord{do} \In{do}, в котором специальные
функции комбинируются так словно они являются обычными.
Для этого используется обратная стрелка. Посмотрим
как определяется функция \In{sequence} в окружении \In{do}:
...
словом \In{do} о том, что выражения записаны в мире монады \In{m}.
Запись с перевёрнутой стрелкой \In{x <- mx} означает, что
мы далее в \In{do}-блоке можем пользоваться значением \In{x}
так словно оно имеет тип просто \In{a}, но не \In{m a}.
Смотрите в этом определении мы сначала извлекаем первый
элемент списка, затем извлекаем хвост списка, приведённый
к типу \In{m [a]}, и в самом конце мы соединяем голову и 
...
В \In{do}-нотации можно вводить локальные переменные 
с помощью \mbox{слова \In{let}}:
...
Посмотрим как \In{do}-нотация переводится в выражение, 
составленное с помощью методов класса \In{Monad}:
...
второго уравнения из функции \In{sequence}
...
С появлением класса \In{Applicative} во многих 
случаях \In{do}-нотация теряет свою ценность. 
Так например любой \In{do}-блок вида:
...
Слово \In{LANGUAGE} говорит компилятору о том, что мы хотим
воспользоваться расширениями с именами \In{ExtentionName1},
\In{ExtentionName2}, \In{ExtentionName3}.
...
имели в виду расширение \In{XXX}. Например попробуйте
...
определена конструкция \In{if-then-else}. Определим тип
...
вроде \In{Add ValTrue (Val 2)} или \In{If (Val 1) ValTrue (Val 22)}.
Наш тип \In{Val} включает в себя все хорошие выражения и
много плохих. Эта проблема проявится особенно ярко, если 
мы попытаемся определить функцию \In{eval}, которая 
...
расширением \In{GADTs}. Помните когда-то мы говорили, что типы
...
параметра \In{a} мы можем писать конкретные типы. Определим
тип \In{GExp}
...
Если \In{eval} получит логическое значение, то будет 
возвращено значение типа \In{Bool}, а на значение типа
\In{Exp Int} будет возвращено целое число. 
...
\In{eval}? Зачем нам сначала кодировать выражение конструкторами,
...
Ещё функция \In{eval} может вычислять совсем другие значения.
...
\In{eval} проецировать разные конструкции Haskell в конструкции
...
в типе \In{Exp} некоторую область и затем 
надстраиваем над типом \In{Exp} разные полезные
функции. На самом последнем этапе функция \In{eval} переводит
...
Например создадим класс-вычислитель \In{Eval}:
...
В классе \In{VectorSpace} мы видим новую конструкцию,
объявление типа. 
Мы говорим, что есть производный тип, который следует
из \In{v}. Далее через двойное двоеточие мы указываем
...
его экземпляром класса \In{VectorSpace}. Для начала
создадим новый модуль с активным расширением \In{TypeFamilies}
...
Теперь определим экземпляр для класса \In{VectorSpace}.
...
воспользуемся расширением \In{EmptyDataDecls}, которое
...
Первой строчкой мы определили семейство функций \In{Add},
у которого два параметра. Определение семейства типов
начинается с ключевой фразы \KeyWord{type family} \In{type family}.
За двоеточием мы указали тип
семейства. В данном случае это простой тип без параметра.
Далее следуют зависимости типов
для семейства \In{Add}. Зависимости типов начинаются
с ключевой фразы \KeyWord{type instance} 
\In{type instance}. В аргументах мы словно пользуемся
...
\In{UndecidableInstances}, поскольку во втором уравнении
мы подставили одно семейство типов в другое. Этот флаг часто
используется в сочетании с расширением \In{TypeFamilies}. 
...
класса \In{Nat}, в котором мы можем переводить типы в числа.
Функция \In{proxy} позволяет нам извлечь значение из
типа-конструктора \In{Succ}. При этом мы нигде не пользуемся
значениями типов \In{Zero} и \In{Succ}, ведь у этих типов 
нет значений. Поэтому в экземпляре для \In{Zero} мы пользуемся
постоянной функцией \In{const}.
...
Параметр \In{n} кодирует точность. Теперь мы можем
...
Это будет специальное расширение \In{TypeLevelNats},
...
Расширение \In{MultiParamTypeClasses} позволяет
...
Расширение \In{TypeSynonymInstances}
позволяет определять экземпляры для синонимов типов.
Мы уже пользовались этим расширением, когда определяли 
рекурсивные типы через тип \In{Fix}, там нам нужно было
определить экземпляр \In{Num} для синонима \In{Nat}:
...
Все они имеют вид \In{T a}. Если мы хотим хотим использовать
суперклассы с составными типами, нам придётся подключить
расширение \In{FlexibleContexts}. Этим расширением мы пользовались,
когда определяли экземпляр \In{Show} для \In{Fix}:
...
\In{NoMonomorphismRestriction}. Это расширение отменяет
...
Без ограничения мономорфизма, для функции \In{f}
будет выведен тип:
...
Это приведёт к тому, что переменная \In{len} 
...
создания парсеров \In{Parsec}. С этим ограничением 
...
\In{Haskell98}. Любое расширение
подключается с помощью специальной прагмы \In{Language}.
Новый стандарт \In{Haskell Prime} включит в себя наиболее
